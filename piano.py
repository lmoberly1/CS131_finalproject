...
def drawButtons(frame, buttonList):
    for button in buttonList:
        x, y = button.pos
        w, h = button.size
        cv.rectangle(frame, button.pos, (x+w, y+h),
                     (255, 255, 255), cv.FILLED)
        cv.putText(frame, button.text, (x + 25, y + 75),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 5)
    return frame


class Button():
    def __init__(self, pos, text, size=[70, 200]):
        self.pos = pos
        self.size = size
        self.text = text
...
for i, note in enumerate(notes):
    buttonList.append(Button([25 + (80 * i), 300], note))
...
class HandEstimator():
    ...
    def detect(self, frame):
        ...
        if hands:
            ...
            # print('hand 1 point: ', centerPoint1)
            # Hand 2
            if len(hands) > 1:
                hand2 = hands[1]
                ...
                # <250 fist is closed and play sound
